...
router.get("/", (req, res, next) => {
  Specy.find({}).exec((err, species) => {
    if (err) {
      res.send({
        message: "specy could not be retrieved because" + err.message,
      });
    }
    res.send(species);
  });
...
router.post("/", (req, res, next) => {
  ...
  specy.save(function (err) {
    if (err) {
      res.send({
        message: "Specy could not saved specy because: " + err.message,
      });
    }
    res
      .status(201)
      .send({ message: "specy created successfully", data: specy });
  });
});
...
router.patch("/:id", (req, res, next) => {
  Specy.updateOne(
    ...
    if (err) {
      res.send({
        message: "Specy could not be updated because: " + err.message,
      });
    }
...
router.delete("/:id", (req, res, next) => {
  Specy.deleteOne({ _id: req.params.id }).exec((err, specy) => {
    if (err) {
      res.send({
        message: "Specy could not be deleted because: " + err.message,
      });
    }
